...
> **状态：已放弃（2026-08-18）**。原因：工具失败/中间过程在真实使用中频繁发生且无需人工介入，通知它们纯属噪音；现有完成/失败/审批/提问 4 类通知已覆盖"需要被打扰"的全部场景。本设计不实现。其中 events.mux 事件格式调研（`assistant/chunk`、`tool/call`、`tool/result` 双层结构等）留档备查，供将来任何 dsh 深集成（如通知内审批/回答）参考。
